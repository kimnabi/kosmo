...
<%
	...
	***********************************/
	...
	if(request.getParameter("curPage") != null){
		...
		System.out.println("curPage222222222222222222 >>> : " + curPage);
	...
	pageCount = (int)Math.ceil(totalCount / (groupSize + 0.0));//end page
	System.out.println("pageCount >>> : " + pageCount);
	//(0/page)
	
	// 현재그룹 설정
	//1-1/5
	int curGroup = curPage-1;
	System.out.println("curGroup >>> : " + curGroup);
	
	//0*0
	int linkPage = curGroup;
...
<%
	...
	// 그룹범위내에서 페이지 링크만듬.
	//5>0 && 1<=40
	while((loopCount > 0) && (tmpPage <= pageCount)){
		//1==1
		if(tmpPage == curPage){
...
<%
			...
			//[2][3][4][5]
%>
	[<a href="<%=url%>?<%=str%>curPage=<%=tmpPage%>"><%=tmpPage%></a>]&nbsp;
<%
		}
